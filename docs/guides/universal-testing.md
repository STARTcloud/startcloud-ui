...
The testing contract: the one browser test tool for the estate and the reusable workflow every UI
...
citation, never with the author. Where the text names "decision n", the Decisions section records
that choice, settled, with the reason it was chosen. Every file this contract adds to the shared UI is
JavaScript, never TypeScript, and no test reads a `.env` file: the environment is what the
workflow or the shell sets. It extends the
...
- **The browser test tool is a normal step of the standard `ci.yml`.** The same shape in every
  repository of its class; Playwright (decision 1). Why: `uniformity.ci_cd.workflow_set`,
...
| Rule                | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Why                                                                                                                            | Spec                                                                     |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| the browser tool    | one browser test tool for the estate, Playwright, decision 1; it is a normal step of each repository's standard `ci.yml`, the same shape in every repository of its class; no repository runs a second browser tool; the shared UI's `playwright.config.js` is JavaScript and reads `process.env` alone, no `.env` file and no `dotenv`                                                                                                                                                                                                                             | the owner's rule; one tool is what makes the tests uniform and convergent                                                      | Playwright Test documentation                                            |
| the Gherkin layer   | Cucumber or Gherkin style on frontends: every browser test of the shared UI is a `.feature` file of scenarios in Gherkin, run through `playwright-bdd`, decision 2, which generates Playwright tests from the features so one runner runs them and one report shows them; a step is written once in a step file and reused by every scenario that says the same thing                                                                                                                                                                                               | the owner's `uniformity.ci_cd.testing` description; a scenario reads as the contract line it proves                            | Gherkin reference (Cucumber); `playwright-bdd` documentation             |
| the unit runner     | one unit runner per class, decision 3: Vitest for the shared UI's `schemaSections`, `validation`, `useFormRules`, `AdminConfig` and `SetupPage`; Jest 30 with supertest for the Node backend; pytest for the Python backend; the Java backend's Gradle test task; Vitest with Cloudflare's Workers pool for the Worker; each named once in section 2 and never a second one in the same class                                                                                                                                                                       | config contract decision 39 names the shared UI's tests; one runner per class is the same-shape rule applied to the unit layer | Vitest, Jest, pytest, Gradle and Cloudflare Workers test documentation   |
| live instance       | on a backend a test runs against the backend's own process booted from the module's own configuration resolution, the config contract's development directory or a `CONFIG_DIR` the test job sets, over a real database of the class's test dialect; nothing the route reads is mocked; how the instance is stood up per class is section 5; the unit layer keeps the in-process app under a request library (BoxVault's supertest over the Express app), and the browser scenarios run against the process listening on a port as `npm start` boots it, decision 4 | "live-instance tests rather than mocks on backends"; a mock proves the mock                                                    | the config contract, section 1 directory row; section 2 development file |
| coverage            | every unit and live-instance run reports coverage through the runner's own coverage reporter; 100 percent is the aim; a branch no request can reach is listed under `uniformity.potential_dead_code` with the reason and is never tested around; coverage is reported on every run and uploaded with the report and gates nothing until a class reaches its first figure, when that figure becomes its floor and never moves down, decision 5                                                                                                                       | the owner's aim and the `potential_dead_code` rule                                                                             | the runner's coverage documentation                                      |
| no smoke script     | no workflow of any repository carries a hand-written smoke script: no inline `curl`, no inline `node --input-type=module`, no `python -c` that asserts a route; a check that must survive becomes a test under the runner of section 2 or a step of the reusable workflow of section 3; the shared UI's locale parity is the unit test `tests/unit/localeParity.test.js`, decision 6                                                                                                                                                                                | the owner's rule; a curl script encodes assumptions nobody maintains and diverges per repository                               | this contract                                                            |
| the screenshot      | the navbar contract asks that every conformance-checklist line be ticked with a side-by-side screenshot in the PR; the browser tool keeps a screenshot, a trace and a video of every failing scenario and uploads them with the run's report as a workflow artifact, and the PR screenshot stays until the line's scenario reads ✓ in section 6, decision 7                                                                                                                                                                                                         | the navbar contract's opening rule; a tool that already drives the page can capture it                                         | the navbar contract; the tool's screenshot documentation                 |
...
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| shared UI      | `tests/` at the repository root: `tests/features/` for the `.feature` files and their step files, `tests/unit/` for the unit tests, `tests/fixtures/` for the fixtures | a feature file per contract, `<contract>.feature` (`config.feature`, `validation.feature`); a step file beside it, `<contract>.steps.js`; a unit test per module it tests, `<module>.test.js` | `tests/fixtures/<contract>/index.js` keyed `"METHOD path"` as `tests/fixtures/config/index.js` is today, one JSON file per answer beside it (section 5)                                                                                                             | Playwright through `playwright-bdd` for the features; Vitest for the unit tests; both as `npm` scripts beside `lint`, `format:check` and `quality`, the CONTRIBUTING chain | the feature-first tree of CONTRIBUTING and the pages contract keeps tests out of `src/`; `tests/fixtures/config/` already exists in this shape                                                                         |
| Node backend   | `backend/tests/` (BoxVault, as today); a backend whose code is at the root uses `tests/`                                                                               | `<subject>.test.js`, as BoxVault's `events.test.js`, `config.test.js`, `rules.test.js` are today                                                                                              | `backend/tests/__test_config__/` holding one `<name>.config.yaml` per name in `status.config`, written by the runner's global setup and handed to the process as `CONFIG_DIR`, as BoxVault's `globalSetup.js` does today; the test dialect `sqlite` with `:memory:` | Jest 30 with supertest, as BoxVault runs today                                                                                                                             | the first backend of the class holds the shape and every later backend copies it, the config contract's one-module rule applied to tests; the test files are the ones the events and validation contracts already cite |
| Python backend | `tests/` at the repository root                                                                                                                                        | `test_<subject>.py`                                                                                                                                                                           | a test configuration directory of one `app.config.yaml`, handed as `CONFIG_DIR`, the Python twin of the Node backend's `__test_config__`                                                                                                                            | pytest, installed from `requirements-dev.txt` beside `ruff` and `pip-audit`                                                                                                | the same shape as the Node class in the language's own convention; `requirements-dev.txt` is where the class already keeps its tools                                                                                   |
...
| Worker         | `worker/tests/`                                                                                                                                                        | `<subject>.test.js`                                                                                                                                                                           | the Worker's `[vars]` for the test run; no configuration files, the config contract's decision 31                                                                                                                                                                   | Vitest with Cloudflare's Workers pool; the live Worker is `wrangler dev` on the runner, decision 8                                                                         | a backend with no files has no `CONFIG_DIR` to hand over; its test shape is its platform's                                                                                                                             |
...
The browser scenarios are the shared UI's alone, over its fixture; no backend writes a `.feature`
file or runs the browser tool, because the backends' proofs are their own unit and live-instance
tests under the runner of their class, as they run today.
...
| Rule              | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Why                                                                                                             | Spec                                                                   |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| one workflow      | one reusable workflow in this repository, `.github/workflows/tests.yml` with `on: workflow_call`, decision 9; every UI backend calls it from its standard `ci.yml` as a job with `uses: STARTcloud/startcloud-ui/.github/workflows/tests.yml@main`, the way `ci.yml` and `release-please.yml` already call `codeql.yml` and `prod-build.yml` within a repository; the ref is `main`, decision 10, because the workflow's shape moves with the contract and a caller pinned to a tag would carry a second pin beside `startcloudUiVersion` for a file that ships no code                                                                                                                                                                | one workflow is what makes the tests convergent; a workflow copied into each repository is five workflows       | GitHub Actions, reusing workflows (`workflow_call`, `uses` on a job)   |
| what it does      | checks out the caller, sets the class's language up, installs its dependencies, fetches the pinned UI release into `ui/` the way every backend's CI already does (`curl -fsSL … \| tar -xz -C ui`, the navbar contract's Distribution section), builds the shared UI, runs the class's unit command, and for the shared UI alone installs the browser tool's Chromium with its system dependencies and runs the features; a caller that names a `boot` command has its live instance booted in the background and awaited on `GET /api/status` before its unit command runs (section 5); the report, the failure screenshots, traces and videos, the coverage and the instance's log are uploaded as one workflow artifact, decision 7 | the steps every class shares are the workflow's; what differs per class is an input                             | GitHub Actions, `actions/upload-artifact`; the tool's CI documentation |
| inputs            | decision 11: `class` (`shared-ui`, `node`, `python`, `java`, `worker`), `runs-on` (`ubuntu-latest`), `node-version` (`22`), `python-version` (`3.11`), `java-version` (`25`, `temurin`), `working-directory` (`.`), `ui-version` (the pinned release to unpack, passed by the caller from its own pin location, empty for the shared UI), `config-dir` (the `CONFIG_DIR` the live instance boots from), `boot` (the background command, empty for a class whose unit runner boots its own app), `origin` (`http://127.0.0.1:3000`) and `unit` (the class's unit command); no input carries a secret                                                                                                                                    | the caller says what it is and the workflow does the rest; a secret in an input is a secret in a log            | GitHub Actions, `workflow_call` inputs                                 |
| the standard step | in every repository of a class the calling job is named `tests`, titled Tests, carries the same inputs, copied not written, so `ci.yml` differs between two Node backends only in the values of its inputs; CodeQL `needs` it beside lint and build; the job runs on every pull request and on the push to `main` through `release-please.yml`'s `ci-checks` job, as every `ci.yml` already runs                                                                                                                                                                                                                                                                                                                                       | `uniformity.ci_cd.workflow_set`; the same shape in every repository of its class                                | the four `ci.yml` files and `release-please.yml` as they stand         |
...
| toasts: own VAPID and SW, re-POST on load, `pushsubscriptionchange`              | the VAPID key route; `/notification-sw.js` served with `Service-Worker-Allowed: /` and `Cache-Control: no-cache`, registered at scope `/` with the app name and the build's version in its query, no `fetch` handler | backend live                    |
...
The branding contract carries no conformance checklist; these lines, derived from its rules, are
its checklist, decision 13:
...
rules, and this contract carries its proofs per page group, decision 14:
...
| how it is served             | the browser tool intercepts every `/api/*` request the shared UI makes, `tests/features/support/fixtures.js`, and answers the fixture's file with its status from the `"METHOD path"` key, a key the fixture lacks answering 404 with a problem body, decision 15; the shared UI is never pointed at a dev proxy target for a test; the scenarios run over the built `dist/` served by `vite preview`                                                     | config contract decision 39, "never a dev proxy target"                                                                              | Playwright's request interception, `page.route`       |
...
| the test dialect             | a Node backend runs `sqlite`; a branch reachable only on MySQL stays listed under `potential_dead_code` until a MySQL run exists, as `search_scope_escape` is today; the workflow adds no MySQL run, decision 16, until the class's first run is green on `sqlite`                                                                                                                                                                                        | the owner's `potential_dead_code` record                                                                                             | `priorities.yaml`, `potential_dead_code.boxvault`     |
| the browser and the backends | no backend runs the browser tool; the browser scenarios are the shared UI's over its fixture, and a backend proves its lines with its own unit and live-instance tests under the runner of its class, decision 17                                                                                                                                                                                                                                         | the backends' tests are theirs and stay in the shape they have today                                                                 | this contract                                         |
...
Settled, in the order they were raised:

1. Settled: Playwright is the browser test tool. It drives Chromium, Firefox and WebKit from one
   API, its test runner carries request interception, screenshots and a CI reporter, it is one
   step in a workflow, and it is the tool the estate's sibling projects already test with; the
   shared UI's config is `playwright.config.js`, JavaScript, reading `process.env` alone.
2. Settled: the Gherkin layer is `playwright-bdd`, which generates Playwright tests from the
   `.feature` files, so one runner runs them and one report shows them; the steps are JavaScript
   step files beside the features.
3. Settled: the unit runner per class. Shared UI: Vitest, because the build is Vite and the runner
   reads the same configuration. Node backend: Jest 30 with supertest, as BoxVault runs today.
   Python backend: pytest. Java backend: the Gradle test task as it runs today. Worker: Vitest
   with Cloudflare's own Workers pool.
4. Settled: the in-process app under a request library, BoxVault's supertest over the Express
   app, is the live instance of the Node class, because it is fast and already there; a class
   whose runner cannot boot its own app names a `boot` command and the workflow boots it and
   waits for `GET /api/status` before the unit command runs.
5. Settled: coverage is reported on every run and uploaded with the report; it gates nothing
   until a class reaches its first figure, and that figure then becomes the class's floor, a
   threshold below the current figure never landing.
6. Settled: the locale-parity check is the Vitest unit test `tests/unit/localeParity.test.js`;
   the inline script is retired from `ci.yml`.
7. Settled: the browser tool keeps a screenshot, a trace and a video of every failing scenario
   of the shared UI and uploads them with the report as a workflow artifact; the PR screenshot
   stays for a line until its scenario reads ✓ in section 6.
8. Settled: the live Worker is `wrangler dev` on the runner; the Workers pool stays the unit
   runner's.
9. Settled: `.github/workflows/tests.yml`.
10. Settled: `main`. The workflow's shape moves with the contract, ships no code, and a tag
    would be a second pin beside `startcloudUiVersion` for a caller to bump.
11. Settled: `class`, `runs-on`, `node-version`, `python-version`, `java-version`,
    `working-directory`, `ui-version`, `config-dir`, `boot`, `origin` and `unit`; the pinned UI
    version is passed by the caller from its own recorded pin location, because every class
    records it in a different file.
12. Settled: the workflow's own install step on every run, on the self-hosted runner too; a
    runner prepared by hand is a runner nobody can rebuild.
13. Settled: the branding lines of section 4 are the branding contract's checklist.
14. Settled: this contract carries the identity proofs per page group; the identity contract
    gains no checklist.
15. Settled: the tool's request interception through `tests/features/support/fixtures.js`.
16. Settled: no MySQL run until the Node class's first run is green on `sqlite`.
17. Settled: no backend runs browser scenarios; the shared UI's scenarios over its fixture are
    the estate's browser tests, and a backend's `tests` job runs its own unit runner through the
    reusable workflow so every `ci.yml` keeps one shape.
18. Settled: the shared UI first, then BoxVault as the source of the Node class, then the
    authorization server, the VDI Health Monitor and the catalog.
19. Settled: the shared UI's scenarios run over the built `dist/` served by `vite preview`,
    never the dev server, because the dev server proxies `/api` to a target and the build is
    what every UI backend serves.
...
| -------------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| startcloud-ui        | shared UI      | `npm test` (`test:unit` Vitest with coverage, `test:features` `bddgen` then Playwright); `tests/unit/` (`validation`, `schemaSections`, `routes`, `localeParity`), `tests/features/<contract>.feature` with `<contract>.steps.js` and `support/`, `tests/fixtures/<contract>/` | `ci.yml`: Lint & Format, Tests (the call to `tests.yml` with `class: shared-ui`), Build, then CodeQL; `release-please.yml` calls `ci.yml`, builds the tarball, publishes `.prepaint.sha256`, dispatches consumers | holds the reusable workflow `tests.yml`; the decision 39 tests for `useFormRules`, `AdminConfig` and `SetupPage` still owed                                                                                                                                                              |
| BoxVault             | Node backend   | `backend/tests/*.test.js`, 53 files, Jest 30 with supertest, `globalSetup.js` writing `__test_config__/` and setting `CONFIG_DIR`, `sqlite` `:memory:`, `TEST_DB_DIALECT` read by the setup                                                                                    | `ci.yml`: Backend Security, Lint & Format, a Test job commented out, then CodeQL                                                                                                                                  | the source of the Node class's shape; the commented Test job replaced by the `tests` job calling `tests.yml` with `class: node`, `working-directory: backend`, `ui-version` from `startcloudUiVersion`, `boot: npm start`, `unit: npm test`; `backend/tests/features/` for its scenarios |
| VDI Health Monitor   | Python backend | none                                                                                                                                                                                                                                                                           | `ci.yml`: Backend Security (`pip-audit`), Lint & Format (`ruff`, `compileall`, Prettier); no CodeQL, private                                                                                                      | `tests/test_*.py` under pytest; the `tests` job calling `tests.yml` with `class: python`, `ui-version` from `pyproject.toml`, `boot` its `uvicorn` command, `unit: pytest`                                                                                                               |
| Authorization server | Java backend   | `src/test/groovy/**/*Test.groovy`, 164 files, `./gradlew test`                                                                                                                                                                                                                 | `ci.yml`: Build & Test on `self-hosted`, fetching the pinned UI tarball, `./gradlew build`, `./gradlew test`; Dependabot Updates; Lint & Format, Build Frontend and CodeQL commented out                          | keeps its tests and runner; the `tests` job calling `tests.yml` with `class: java`, `runs-on: self-hosted`, `ui-version` from `packaging/config/ui-version.yaml`, `boot` the jar, `unit: ./gradlew test`; `src/test/features/` for its scenarios                                         |
| Provisioner catalog  | Worker         | none                                                                                                                                                                                                                                                                           | `ci.yml`: JSON schema and Sorted (Python scripts), Workflow Lint (actionlint), Lint & Format (markdownlint, Prettier), Docs Build & Link Check (Jekyll, html-proofer), then CodeQL                                | `worker/tests/` under Vitest with the Workers pool; the `tests` job calling `tests.yml` with `class: worker`, `working-directory: worker`, `boot: npx wrangler dev`, `unit: npx vitest run`; every config line n/a                                                                       |
...
Playwright Test documentation (the runner, request interception with `page.route`, screenshots,
the CI guide and browser installation) · `playwright-bdd` documentation · GitHub Actions: reusing
workflows (`on: workflow_call`, `jobs.<id>.uses` with `owner/repo/.github/workflows/<file>@<ref>`,
inputs, access from private repositories to a public repository's workflow),
`actions/upload-artifact`, self-hosted runners · Vitest, Jest, pytest, the Gradle test task and
Cloudflare's Workers test documentation · the Universal Config Contract (section 1 directory row, section 2 development file, decision
