...
| `register`, `recovery`, `name`, `terms` on the identity provider | `POST /registration`, `POST /passwordRecovery`, `POST /complete-onboarding/name`, `POST /api/admin/terms`, `POST /api/admin/terms/{name}/publish` | `email` (`$defs.email`, required) for the two address forms, the address carrying one name everywhere on the issuer; `given_name` (`$defs.personName`, required) and `family_name` (`$defs.personName`) for the onboarding name step; the terms record: `name` (`$defs.slug`, `unique` global, required), `friendly_name` (`maxLength 255`), `icon` (`$defs.iconName`), `content` (`type: string`, required on create, optional on publish), `version` (`maxLength 50`, required on create and on publish, `unique` within the copy on publish, a taken value `409` as every `unique` is, never a member of `PATCH /api/admin/terms/{name}`, which refuses it `422` `readOnly` at `/version`, identity contract decision 161), `type` (`enum` `SITE`, `CLIENT`, `BOTH`, required), `is_public` (`boolean`), `regions` (`array` of `$defs.region`, optional, the copy's regions, empty on the default); the `preferences` form on `PATCH /api/user/preferences`: `language` (`$defs.languageTag`), `theme` (`enum` `light`, `dark`, `auto`), `timezone` (`$defs.timezone`), `ciba_channel` (`enum` `PUSH`, `EMAIL`, `SMS`), `ciba_user_code` (`minLength 4`, `maxLength 64`), `region` (`$defs.region`, optional); the issuer lists its own forms because its registration takes an address alone, and its `password` form carries `maxLength` from its own policy, 64 by default |
...
| `format`                 | `uri`, `hostname`, `ipv4`, `duration` (an ISO 8601 duration, `PT1H`, `P7D`, evaluated as the pattern `^P(?!$)(?:\d+D)?(?:T(?=\d)(?:\d+H)?(?:\d+M)?(?:\d+(?:\.\d+)?S)?)?$`) and `ttl` (the Spring style, `30m`, `4h`, `2d`, evaluated as `^\d+(ms\|s\|m\|h\|d)$`), the last two the Universal Config Contract's duration control; the spec makes `format` an annotation by default and permits assertion (§7.2), and this estate's evaluator asserts it; the address grammar is the named `$defs.email` pattern rather than `format: email`, because the spec's `email` is RFC 5321 §4.1.2 and a stock validator would disagree with the HTML grammar this estate uses | §7                   |
...
| `readOnly`    | the member is answered but never written by this route, the config contract's rule for a `PUT` carrying a `readOnly` pointer (its decision 43) and the identity provider's for `version` on `PATCH /api/admin/terms/{name}` (its decision 161); 422 | none             |
...
| `region`       | `^[A-Z]{2}$\|^(EU\|EEA\|UK)$`                                                                                                                     | an ISO 3166-1 alpha-2 country code or one of the three named sets a terms variant may target                                                                                                                                                                     |
