...
reads, left to right, Need help?, Email support, the language control,
the theme button, then the Sign in button of the Account row, each
present only by its own condition; signed in it reads search, theme,
language, account.
...
| Need help?    | a text link, "Need help?": while `ticket` in the status payload is non-null, the improvement-request ticket URL opened in a new tab, built from `ticket.baseUrl` with `req` from `ticket.reqType`, `customerId` from `ticket.fallbackCustomerId` and `context`, and no `user` or `email`, because the customer id of the whole app rides the status payload so an anonymous visitor on the sign-in, recovery or error pages can report a fault against the site's customer, a six-character hex code Prominic identifies its customers by; while `ticket` is null and `links.docs` is set, `links.docs`; absent when neither, because a help link that lands nowhere is worse than none | hidden; the Help row of the user menu is the same ticket URL with the person's customer id, name and email added, and `links.docs` is the Docs row of the app section                                                                                                                                                                                                                                          |
| Email support | a text link, "Email support", the `mailto:` of `links.contact`; absent while `links.contact` is empty                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | hidden; `links.contact` is the Contact row of the app section                                                                                                                                                                                                                                                                                                                                                  |
...
| Account       | primary **Sign in** button after the theme button; it always carries the page it was pressed on as the return path, the ended session's page while the session-ended banner shows, never an auth page; hidden while the route is an auth path of the identity contract, where the page itself is the sign-in                                                                                                                                                                                                                                                                                                                                                                            | user's name + avatar as one toggle; clicking either opens the menu; the account button carries the unread count on its top right, the same number the menu's Notifications row shows, hidden at zero, from the notifications feature's one context, in the sidebar Inbox badge's look, because the avatar is the one control on every page and a person must see that something waits without opening the menu |
...
- Nothing sits left of the cluster but the brand while signed out, and
  ...
  `links.contact` are the cluster's Need help? and Email support while
  signed out and rows of the app-named menu section signed in, and About
  is a menu row alone, because a signed-out page is a sign-in page or a
  public listing and every link on it that is not the brand is help.
...
While signed out the cluster's Need help? link builds the same URL with
...
| Cluster: search · theme · globe · name+avatar signed in; Need help? · Email support · globe · theme · Sign in signed out, the brand alone on the left                                      | ✓                                                                           | ✓                                                                                                                                                  | ✓ under `idp`; no Sign in button under `auth: []`                                                                 |                | to come — the shared chrome; the Sign in button hidden on the auth routes, Need help?, Email support, language and theme present there                                                                                   |
...
| `links.docs` and `links.contact`: Need help? and Email support in the cluster signed out, Docs and Contact rows of the app section signed in; no search icon signed out                    | ✓ Contact/Docs                                                              | ✓ Docs                                                                                                                                             | ✓ Docs                                                                                                            |                | to come — `help_url` and `mailto:<support_email>`                                                                                                                                                                        |
...
| Need help? in the signed-out cluster is the ticket link while `ticket` is non-null, the fallback customer id alone, in a new tab; `links.docs` while `ticket` is null                      | to come                                                                     | to come                                                                                                                                            | to come                                                                                                           |                | to come — the auth pages, `ticket.fallbackCustomerId` the site's `customer_id`                                                                                                                                           |
