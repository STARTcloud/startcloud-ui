...
- The identity provider's `/userinfo` is the source of identity, favorites,
  organizations, and preferences. Server-side apps proxy it with the OIDC
  access token they hold for the user; public SPAs call it directly.
...
shapes, `/badge`, `/scim/v2`, `/catalog.json`). A machine and a browser
...
```json
{
  "role": "boxvault",
  "version": "0.77.0",
  "brand": { "name": "BoxVault", "logoUrl": "/brand/boxvault.svg", "repo": "https://github.com/Makr91/BoxVault" },
  "auth": ["backend"],
  "collections": ["boxes", "isos"],
  "features": ["local-accounts", "setup", "admin", "org-console", "discover", "invitations", "uploads", "watches", "deploy", "favorites", "notifications", "health", "footer"],
...
| `brand` | `name` (the app section's header and the footer's name), `logoUrl` (a path the UI backend serves, the brand mark, the org mark and the favicon), `repo` (the footer link) and optionally `theme`, the site's default variant (`light` or `dark`) the pre-paint script applies before local storage holds a choice |
...
| — | divider | | |
| 4 | **Favorites** header + one row per app | `favorite_apps` non-empty | Sorted by `order`; label `customLabel` → `clientName` → `clientId`; icon `iconUrl` → `{homeUrl origin}/favicon.ico` → star; opens `homeUrl` in a new tab with `rel="noopener noreferrer"`. A `homeUrl` or `iconUrl` is drawn only when it parses with the `https:` scheme, and every image carries `referrerpolicy="no-referrer"`, because these values come from client registrations and a `javascript:` or `http:` value would run or leak on every app that draws the menu. Favorites are managed on the IdP profile page, and an app may offer "Add to Favorites" on its own About page. |
| — | divider | | |
...
| `footer` token listed | to come | to come | to come | | to come — per site, `sites.<id>.ui.footer` |
...
| Favorites: order, label chain, icon chain, new tab | ✓ from userinfo | ✓ | ✓ from userinfo | | to come — from `GET /api/user` `favorite_apps` |
