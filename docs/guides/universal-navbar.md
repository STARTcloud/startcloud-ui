...
the theme button, the language control, then the account control, each
present only by its own condition, so signed out it reads Discover, the
ticket icon, theme, language, Sign in, and signed in search, Discover,
...
| Theme    | icon button, cycles auto → light → dark, before the language control; while the host answers `brand.packs` the button is instead a menu: the variant rows, "Follow this site" first while the host names a default variant (the state a person is in before choosing, returned to as a cleared `theme`), then auto, light and dark, then a Look section with "Follow this site" and one row per pack carrying that pack's own `logo` and `label` from `brand.packs`, hover or focus previewing the look live and a click keeping it                                                                                                                                                                          | same, drawn between search and language; writes through `PATCH /api/user/preferences`, the look as `pack`                                                                                                                                                                                                                                                                                                      |
...
| `src/components/common/`                                                       | `Avatar.jsx`, `BrandLogo.jsx` (the `status.brand.logo_url` mark, the file painting its own light and dark variant), `ErrorBoundary.jsx`, `NotAvailableStub.jsx`, `ConfirmModal.jsx`, `PageHeader.jsx` and the other pieces every feature draws                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `brand`           | `name` (the app section's header and the footer's name), `logo_url` (a path the UI backend serves, the brand mark, the org mark and the favicon), `repo` (the footer link), optionally `theme`, the site's default variant (`light` or `dark`) the pre-paint script applies before local storage holds a choice, and optionally `pack`, `{ name, css }`, the pack the shell stamps as `data-brand` and links as its stylesheet, per the [branding contract](preferences-and-branding/); absent means stock Bootstrap; optionally `packs`, `[{ name, css, label }]`, the packs a person may choose on this host in the host's order, each `css` the stylesheet URL on the serving origin as `pack.css` is, the shell completing every row by name from the build's own `public/themes/packs.json` (the manifest the generator writes from every pack's YAML: `label`, `description`, `brand` and `logo`) before it boots, so a host names which packs it offers and the pack supplies its own words and mark, the shell drawing its Look picker from that completed list alone and the person's choice persisting as the `pack` preference; absent means every pack of the build is offered in the manifest's order, an empty list none, and a list exactly those, so a host exposes everything by default and a site exposes only what its key lists |
...
| `links`           | `docs` and `contact`: the Docs and Contact rows of the app section signed in, each absent while empty; neither draws in the signed-out cluster; optionally `community`, a list of `{ label, url }` the host answers per site, each one community or support link the About page draws after its repo, changelog and contact links in the host's order (a sponsorship page, a maintainer profile, a forum), `label` the host's own text drawn as it is and never translated, like `brand.name`, and `url` an `https:` URL; the UI drops an entry whose `url` is not `https:` or whose `label` or `url` is not a string, draws nothing for a member that is not a list, and nothing throws                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- `src/components/layout/` — the whole chrome as one folder: `Header.jsx`
  ...
  in one order in both states, search, Discover, the ticket icon, theme,
  language, then the account menu or Sign in, search and the menu signed
  ...
  route alone), `LookMenu.jsx` (the theme control while the host offers
  packs: the variant rows and the Look section with live preview),
  `UserMenu.jsx` with
...
- the pre-paint script in `index.html` — resolves account preference →
  `localStorage.theme` → `auto`, stamps `data-bs-theme` and `lang`, and
  paints the person's chosen pack from `localStorage.pack` and
  `localStorage.packs`, `data-brand` and the pack's `<link>`, over the
  host's own before first paint.
- the theme state: `themePreference` stored under `localStorage.theme`,
  `auto` resolved through `prefers-color-scheme` via
  `useSyncExternalStore`, write-through on toggle; the look beside it,
  the chosen pack under `localStorage.pack` and the host's offered list
  under `localStorage.packs`, painted through `applyPack` of
  `src/lib/runtime.js` and written through as the `pack` preference.
