...
| Ticket   | a life-ring glyph (`FaLifeRing`), `aria-label` "Report a problem", shown only while `ticket` in the status payload is non-null; opens the ticket URL in a new tab built from `ticket.baseUrl` with `req` from `ticket.reqType`, `customerId` from `ticket.fallbackCustomerId` and `context`, and no `user` or `email`, because the customer id of the whole app rides the status payload so an anonymous visitor on the sign-in, recovery or error pages can report a fault against the site's customer, a six-character hex code Prominic identifies its customers by | hidden; the Help row of the user menu is the same URL with the person's customer id, name and email added |
...
While signed out the cluster's ticket glyph builds the same URL with
...
| Ticket glyph in the signed-out cluster while `ticket` is non-null, the fallback customer id alone                                                                                          | to come                                                                     | to come                                                                                                                                            | to come                                                                                                           |                | to come — the auth pages, `ticket.fallbackCustomerId` the site's `customer_id`                                                                                                                                           |
