...
- Left of the cluster, signed in, the app draws a breadcrumb after the
  ...
  mark) and name, a collection crumb its icon. On a collection's own root,
  `/<segment>` or `/<key>` for the segment-less collection, the crumbs
  read the root crumb (the brand without a column) then the collection,
  the collection crumb linking to that root; an organization's pages keep
...
- **Entries.** Two kinds share the one column. Sections: an uppercase
  ...
  route crumbs down the tree before the node was ever expanded; the
...
- **Export.** A feature exports a `sidebar` function,
  ...
  `sidebar(account, collections)` for the catalog feature, which draws
  the Browse tree for every visitor of a host that lists `sidebar` and
  mounts a collection, answering
...
| `sidebar`                                                                 | the sidebar column: every sidebar group and tree the mounted features export, the badges and the root crumb; without it no column and the brand stays in the header                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
2. The catalog's Browse tree gives every mounted collection a root node
   ...
   root › collection, the collection crumb linking to that root, and an
