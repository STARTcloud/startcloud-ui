...
- **One visibility rule per app.** What a member sees is decided once:
  ...
  four lines: public and published, anyone can access, period; published
  and private, anyone in the same organization can access; published,
  private and marked `guest_access`, the organization's guests can access
  it too; unpublished, no one but the user who uploaded it can access,
  not even people in their organization. A guest of an organization, the
  read-only fourth role of the
  ...
  Download button staying, and the profile's Service accounts section is
  absent while every membership is guest and the viewer is no global
  admin. `isMember`
...
- Parse the path with the mounted collections: `org`, `name`, `version`,
  ...
  column the crumbs are the group and the row a sidebar row matches, the
  group a plain word and no root crumb before it, the column's top being
  the way home; a route a child row matches draws the group, the parent
  ...
  decision 130); a page names its own crumb through `usePageName(name)`,
  writing the crumb context the app mounts around the shell, a
  `CRUMB_PARENTS` entry naming a route pattern (`/admin/users/:id`) as
  readily as a path and its resolver receiving
  `{ activeOrganization, pageName, params, t }` and reading `pageName`
  first, so the user record page reads Admin, Users, then the username,
  the translated `admin.users.placeholder` ("User") standing in while the
  record has not answered; a route no row matches draws the route
  parser's crumbs, never the tree's, a node's `matches(pathname)` opening
  the tree alone on a deep link; a reserved route no row matches draws
  the page's title as its one crumb, so the row is never empty.
...
- **Bulk on every table.** Every listing and detail table of every
  ...
  actions for that level (Make public, Make private, Allow guests, Deny
  guests, Publish, Unpublish, Delete on items; Deprecate and Delete on
  versions; Delete on providers, architectures, releases, patches and
  files), then the
...
select checkbox, the watch star and the Deploy glyph are the only in-row controls. One label key per column,
...
by their lower-cased text, dates and counts by value, Visibility public,
...
collection listings with their watch star and organization group rows,
the detail pages, the admin lists, the fleet, the search page and the
organization console's lists, so a table drawn with its own markup is a
defect. A column's `when` receives `(rows, ctx)`, so a column reads the
viewer and the host from `ctx` as well as the rows; a table with no rows
and no groups draws the `EmptyState` placard inside its wrap in place of
the whole table, titled by its `emptyText` with `emptyBody` under it, no
column group and no header, and a group with no items keeps its heading
over one compact placard row. Every column resizes: each header cell but
the select, star and Actions cells carries a handle straddling
...
- `published: null` and `isPublic: null` hide the matching chip and
  column; `guestAccess` is read beside a false `isPublic` alone, true
  drawing Guests and false Private; `downloads: null` on an item and
  `downloadCount: null` on a file hide the Downloads column and the count
  in the Download button the same way, and BoxVault answers both as
  `null` on a download to anyone who is not a member of the organization
  and to a guest on every collection, because a public file's tally is
  the organization's own fact and nobody outside it has need of it. A
...
collection {
  ...
  icon           component; the crumbs, the listing heading, the search results and the
                 Browse tree's root node render it
...
- **PageHeader**: media (artwork), title with an optional trailing
  ...
  published/pending, public/guests/private, OS, deprecated), an action pane on
...
- **Downloads on the same pages**: the ItemPage lists a product's releases
  ...
  `release`, `patch` and the zone's `is_public` and `guest_access`
  creates the absent levels, moves the file in and
...
- **ProfilePage**: the account's own page for an app with accounts of its
  ...
  and BoxVault alike,
  `ProfilePage({ session, events, returnTo, account, basePath, activeOrgUuid, organizations, admin, user, loaded })`:
  `organizations` is the session's memberships in the chrome's shape,
  `admin` the app's global-admin flag, and `user` and `loaded` are the session state's adopted user and whether
  ...
  `account` adapter carries its calls, the one list
  `sectionsFor(account, memberships, admin)` answers: Profile always (one details form on every host, its fields the
  ...
  `serviceAccounts`, Service accounts also only while one of the
  session's memberships holds a role beyond guest or the viewer is a
  global admin, because a guest may not create one. The adapter carries one word beside its calls,
  ...
  each membership row drawing the organization's logo through the shared
  `OrgLogo`, from the session's organizations matched by name, the row's
  own `https:` `logo_url` else and the building glyph otherwise, Make
  primary drawn while `setPrimary` is carried, and `serviceAccounts` is
  `{ list, organizations, create, remove }`, the create form in a
  `SectionCard` with its role select drawn from the host's
  `serviceAccount` rules enum, `superadmin` offered to a global admin
  alone, the one-time token notice, then the keys in the one
  ...
  same `sectionsFor` over the host's `profile` adapter, the account's
  organizations and its global-admin role, and the host's
...
discover, join, gravatarProfile`).
  ...
  `default_role`, the read-only profile of an IdP-managed one, its
  fields as the shared `RecordRows` with the provider link under them,
  the members as `UserCard` rows with role and removal
...
| Visibility           | rows that are not public exist on the page                                | Public, Guests, Private                                                                                                                                                                                                                                                                                                                                                                       |
...
[Universal Navbar Contract](universal-navbar/#sidebar) exports a
`sidebar` function beside its pages, adapter and API calls,
`sidebar(status, account)` for a feature gated by the host's tokens; the
...
catalog feature exports `sidebar(status, account, collections)`: on a
host that mounts a collection, one Browse group holding the Browse tree
of `useCatalogTree` alone, the column's brand link being the way home,
drawn for every visitor while the host lists `browse` and for a
`ROLE_ADMIN` account alone while it lists `admin` instead, else nothing;
one node per mounted collection, carrying the collection's `icon`
component, routing to that collection's own all-organizations listing,
`/<segment>` or `/<key>`, never to the home page, open on that route,
and folding
...
every node carrying `matches(pathname)` over the shared `parseRoute`
...
`<group> › <row>` as its crumbs, and a route a child row matches
`<group> › <row> › <child>`, since its first segment is reserved and
...
| Path                                                          | Role                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/features/catalog/`                                       | The shared pages: `HomePage.jsx`, `OrgPage.jsx`, `CollectionPage.jsx`, `ItemPage.jsx`, `VersionPage.jsx`, `ProviderPage.jsx`, `Listing.jsx`, `ItemsTable.jsx`, `ItemCards.jsx`, `ItemFacts.jsx`, `ChecksumCell.jsx`, the `useCatalogSearch` hook, `sidebar.js` (the Browse group) and `hooks/useCatalogTree.js` (its Browse tree)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `src/utils/routes.js`, `src/components/layout/AppShell.jsx`   | The route parser, `reservedSegments(collections)`, and the crumbs drawn from the route, a sidebar row or the page-name crumb context (`src/contexts/CrumbContext.jsx`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
