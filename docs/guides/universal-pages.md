...
- **SectionCard**: the one section card every page draws its sections
  ...
  Preferences card, the organization console's sections, the About
  page's blocks and `ConfigSections`' section heads are one card with one
  fold, because a person who learns to fold one section has learned to
  fold every section and a fold that lives in the page's prefs survives
  the reload the way the page's sort and hidden columns do.
