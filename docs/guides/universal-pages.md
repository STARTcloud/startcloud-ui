...
organization owns items, an item has versions, a version has providers, a
provider has architectures or artifacts, and a single-file item (an ISO)
has one artifact. BoxVault serves boxes and ISOs that way; the provisioner
...
- **One table per collection**, organization rows as group rows inside it,
  ...
  non-deprecated version. The collection's `deploy.jsx` supplies only the
...
Latest release 9.3rem, OS 10rem, Size and Tier 7rem; header cells never
wrap), so the shared columns sit at the same x on every table of both
...
ascending pass. The Checksum column shows the whole hash, ellipsized by
the column's width with the full value in its tooltip. A column may carry
...
item {
  ...
  artifact { fileName, fileSize, checksum, checksumType, downloadUrl, downloadCount } | null,
...
- `artifact` is the single file of a single-file item. An ISO is an item
  with `artifact` set and `versions` empty; the ItemPage then shows the
  hero, the actions and a Facts card of size, checksum, created, updated
  and downloads, and no versions table. The ISO row gains a
  `downloadCount` column in BoxVault, incremented by the ISO download
  controller.
...
- **ItemPage**: PageHeader (its action row opens with the filled Deploy
  ...
  the facts panel (from `metadata` or `artifact`)
  ...
  `hasVersions: false`; its actions (Download, Copy checksum, Make public
  / Make private, Publish / Unpublish, Rename, Delete) are the
  `ItemActions` slot.
- **VersionPage**: PageHeader with the deprecated chip, the deprecation
  ...
  where a version has artifacts, the providers table (name, description,
  downloads per architecture, actions slot).
...
`auth` namespace. A feature imports from `components/`, `hooks/`, `contexts/`,
`lib/`, `utils/` and its own folder, never from another feature except
the collections registry and `features/deploy`; the pages name no
namespace but `shared` and `auth`. A feature's heavy libraries (a map, an
