...
| table preferences                 | `table_prefs_<org or home>`, one JSON object `{ view, sort, hiddenColumns, filters, folds }` per key, `view` present only on a page with the view toggle, so a page's choices travel as one value and a key never holds a bare string | `table_prefs_<org or home>`, one JSON object `{ view, sort, hiddenColumns, filters, folds }` per key, `view` present only on a page with the view toggle, so a page's choices travel as one value and a key never holds a bare string |
