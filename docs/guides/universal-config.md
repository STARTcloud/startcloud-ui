...
| a changed rule         | the tightened rule applies at boot; a value that no longer passes stops the UI backend with its pointer, so an upgrade never runs on a value the new release cannot honor                          |
...
| `schemaSections` | `src/utils/schemaSections.js`: `schemaSections(schema)` walking the schema into sections, `fieldOf` for one property, `valueAt(config, pointer)` and `setValueAt(config, pointer, value)` over the plain file; the labels come from `title`, the key only when there is none                                                                                                                                                                                                                                                                                       |
