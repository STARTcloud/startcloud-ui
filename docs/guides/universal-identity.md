...
`pack`, `packs` and `language` (the chrome's own keys, `pack` the chosen
look's name and `packs` the host's offered list cached for the pre-paint
script), `table_prefs_admin_users`,
...
| `ticket`                        | `base_url` from `integrations.improvement_request.base_url`, `req_type` its `req`, `fallback_customer_id` the site's `customer_id`; `null` while `improvement_request.enabled` is false                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | when neither the active organization nor the person carries a customer id, the ticket belongs to the site the person was on, not to the global `improvement_request.customer_id`                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `password_reset_enabled`, `sign_in_link_enabled`   | whether the page draws the Forgot password link and whether the emailed sign-in link is offered as a mode; both open unless the answer says false, registration alone closed until its member opens it (`signInAffordances`); the site decides what exists on the page, and the client whose authorize request is parked in the session may only hide or reorder inside that, never add, through its `clients.<id>.client.hide-password-reset` and `hide-sign-in-link`, the site's answer as it is while none is parked; `remember_me_enabled` and `local_registration_enabled` are narrowed the same way by `hide-remember-me` and `hide-registration`, `mark_enabled` is true only while the parked client's `show-mark` is on, the page then drawing the site's own mark from `brand.logo_url` above its heading and nothing else changing, `hide-password` and `hide-passkey` drop those methods from `methods[]`, and `login-mode` names the mode shown first, moved to the other mode when it points at a method the client hides; because a client such as BoxVault's downloads face wants a plain sign-in for its own flow while every other client on the same site keeps the full page | `AuthMethodsApiController`, the parked `/oauth2/authorize` request's `client_id` from the request cache           |
...
| `mark_enabled`                                     | true only while the parked client's `show-mark` is on; the page then draws the site's own mark, `brand.logo_url` of `/api/status`, above its sign-in heading and nothing else changes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `AuthMethodsApiController`, the parked client's `show-mark`                                                       |
...
that client's flow; `show-mark` is the one addition and turns its member
true; with no request parked every member is the site's own answer.
...
| `show-mark: true`               | `mark_enabled: true`                            | draws the site's mark above the sign-in heading |
...
| preferences                | `PATCH /api/user/preferences` with `language`, `theme`, `pack`, `timezone`, `region`, `ciba_channel` and `ciba_user_code`; `pack` a bare pack name the site offers in `brand.packs` or `null` to follow the site's own, `422` `enum` at `/pack` for any other; `region` the person's legal region, a two-letter ISO 3166-1 code or one of `EU`, `EEA`, `UK`, `null` clearing it, the value the terms page's region button writes and the terms and policy copies resolve to first (`422` `enum` on `/region` otherwise); `ciba_user_code` a string that sets the approval PIN and `null` that clears it, never read back, `ciba_user_code_set` being the read's word for it; its `400 { error }` becoming `422` with pointers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | unchanged                                                                                                        |
...
169. The sign-in page's switches are per client, never per site:
     ...
     `ui.default_login_method`, `tos-names`, a provider's `allowed-sites`
     and `allowed-networks`) and a client may only hide or reorder inside
     that, never add; the one thing a client may add is the site's own
     mark above the heading, `show-mark` answering `mark_enabled` true so
     the page draws `brand.logo_url` there, because a plain page for one
     application may still want the brand it belongs to over its heading;
