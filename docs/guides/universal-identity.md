...
- **GET is the page, POST is the action.** Every page GET on the issuer is
  ...
  other protocol endpoints; every other GET is a page.
...
- **Redirects stay where a browser must follow them.** The federated hop
  ...
  `/login/magic`, `/registration/verify` and `/org/invite/:token` are SPA
  routes whose page posts `{ email, token }` to consume the token, and
...
- **Nothing sent from the page is a secret the page keeps.** The CSRF
  ...
  the profile (`name`, `email`, `picture`, `roles`, `organizations`, never
  the address, the birthdate or the preferences) and the chosen sign-in
  mode. Every POST on the issuer, `POST /login` included and whatever
  ...
  backup-code list or an enrolment key is never kept by a cache on the
...
`table_prefs_admin_logins`, `table_prefs_admin_registrations`,
`table_prefs_admin_sessions`, `table_prefs_inbox`, and the sidebar keys
...
```json
{
  "role": "auth-server",
  "version": "1.9.1",
  "brand": { "name": "STARTcloud", "logoUrl": "/brand/startcloud/icon.png", "changelog": "https://github.com/STARTcloud/authorization-server-private/releases", "theme": "light" },
  "auth": ["cookie"],
  "collections": [],
  "features": ["local-accounts", "tfa", "onboarding", "interstitials", "policies", "org-console", "invitations", "integrations", "inbox", "admin", "notifications", "health", "events", "footer"],
  "links": { "docs": "", "contact": "" },
...
| `version` | the Spring Boot build-info version (`BuildProperties`), the number release-please writes into `build.gradle` | one number, read at runtime from the artefact that is running, the way every other UI backend answers its own released version; never a value in the UI build, never a second file to bump |
...
- **The UI never enters this repository or the jar.** The pinned release
  ...
  never from the request's `Host`, an unrecognised host answering the
...
for SMS and APP, a `role="group"` of six boxes labelled "Digit n of 6",
...
attempts; use another and re-enrol it from your profile."; the preferred
...
| --- | --- | --- |
| `CodeInput` | `src/components/common/CodeInput.jsx` | a one-time code entry any UI backend with a second factor or an email verification code draws; the `[data-otp]` behaviour of `auth.js` |
...
`continue`, `invalid`); `errors.*` gains one key per `code` above. Every
...
| `/complete-onboarding/choose-2fa-method` | TfaEnrolChoiceStep | the same, `tfa` | `POST /complete-onboarding/choose-2fa-method` |
| `/qrcode` | TotpEnrolPage: the QR, the setup key, the code, the onboarding chain's only; the profile enrols an app inline on its Security tab so a signed-in person never leaves the tabs | `cookie`, `tfa` | `GET /api/auth/tfa/enrol`, `POST /qrcode/verify` |
...
```json
{
  ...
  "phone": { "purpose": "verify", "resend_after_seconds": 30, "policies": [ { "name": "privacy", "label": "Privacy Policy", "url": "/public/policies/privacy" } ] },
...
| `phone.purpose` | `verify` on a local signup (a real-person check, no 2FA enrolment, no "choose another method" link) or `tfa` on a federated one (SMS 2FA enrolled on success, backup codes follow) | `isLocalSignup` |
| `tfa.verified_phone` | the masked number when SMS 2FA can enrol without retyping | `verifiedContactPhone` |
...
`GET /api/auth/tfa/enrol`: `{ "qr": "data:image/png;base64,…", "secret": "JBSW…", "issuer": "STARTcloud" }`,
...
```json
{
  ...
  "fields": [
    { "param": "first_name", "label": "First name", "autocomplete": "given-name", "group": "identity", "control": "text", "span": "half", "required": true },
    { "param": "country", "label": "Country", "autocomplete": "country-name", "group": "address", "control": "country", "span": "half", "required": true }
  ],
...
account lacks; `fields` lists those, in render order; the three HTML
...
| resend the email code | `POST /complete-onboarding/email-verification/resend`, no body, so a person whose code never arrived has a way forward | `429` `throttled` with `wait_seconds` |
...
- **OnboardingHub** at `/complete-onboarding` reads the state and
  ...
  labelled "Step n of m" with a hidden step name so a screen reader hears
...
- **NameStep**: given names and family name, the family-name hint,
  ...
  (`pattern: personName` joins `$defs`), the autofill tokens
...
- **TfaEnrolChoiceStep**: `OptionList` with Authenticator app
  (recommended) and SMS (the verified number line when
  `tfa.verified_phone`, the risk notice when `sms_risk_notice`).
- **TotpEnrolPage** at `/qrcode`: the QR image, "Unable to scan?" with
...
`onboarding.*` (`welcome`, `subhead`, `password`, `confirm`,
...
`phone.subheadTfa`, `phone.found`, `phone.send`, `phone.code`,
`phone.codeHelp`, `phone.verify`, `phone.resend`, `phone.resendIn`,
`phone.consent`, `phone.chooseOther`, `email.title`, `email.sent`,
`email.code`, `email.verify`, `email.notReceived`, `tfa.title`,
`tfa.subhead`, `tfa.app`, `tfa.appHelp`, `tfa.sms`, `tfa.smsHelp`,
`tfa.smsVerified`, `qr.title`, `qr.scan`, `qr.unableToScan`,
`qr.setupKey`, `qr.verify`, `codes.title`, `codes.important`,
`codes.body`, `codes.copy`, `codes.download`, `codes.noViewAgain`,
`codes.confirm`, `codes.continue`, `account.title`, `account.subhead`,
`account.requiredBy`, `account.personal`, `account.personalHelp`,
`account.team`, `account.teamHelp`, `team.title`, `team.subhead`,
`team.name`, `team.create`, `team.back`); `terms.*` (`before`,
`version`, `step`, `fontSize`, `accept`, `agree`, `yourDetails`,
`phoneVerification`, `address`, `acknowledge`); `policy.*` (`version`,
`updated`, `created`, `return`); `errors.*` gains `session_expired`,
...
```json
{
  ...
  "scopes": [
    ...
    { "id": "organizations", "label": "organizations", "description": "See your organization memberships and roles" }
  ],
  "authorization_details": [
    { "type": "domino_vault", "description": "Access an ID Vault", "locations": ["https://vault.example"], "actions": ["read"], "datatypes": null, "identifier": null, "privileges": null }
  ]
}
```

`scopes` lists only the scopes not yet granted, labelled from the
...
call's reported URL: `{ "client_name", "binding_message", "scopes": […], "authorization_details": […] }`,
...
`scope.offline_access` and one key per standard scope the issuer
offers); `device.*` (`title`, `subhead`, `code`, `continue`,
...
`invalid`); `link.*` (`title`, `exists`, `either`, `password`, `code`,
`confirm`, `cancel`, `finePrint`); `errors.*` gains `bad_password`,
...
| `/user/profile` (and `/profile`) | ProfilePage in its issuer form, five tabs: Profile, Security (`#security`), Preferences (`#preferences`), Favorites, Sessions; the shared Organizations tab is not drawn on the issuer because the sidebar's Organizations row is the same destination | `cookie` | `user/profile.html`, `UserController`; `/.well-known/change-password` redirects to `/user/profile#security`, the page the W3C well-known URL must lead to, in place of the template's anchor |
| `/user/organizations` | OrganizationsPage: the memberships as cards, create a team, join by code; a card opens the console for that organization | `cookie`, `org-console` | `user/organizations.html`, `OrganizationController` |
...
| `/org/invite/:token` | a top-level navigation the mail carries; the server consumes and redirects to `/user/organizations` | none | `OrganizationController.consumeInvite` |
...
```json
{
  ...
  "address": { "line1": "", "line2": "", "city": "", "state": "", "postal_code": "", "country": "", "country_code": "", "formatted": "", "latitude": null, "longitude": null },
  "has_local_auth": true,
  "requires_password_setup": false,
  "tfa": { "enabled": true, "preferred_method": "APP", "preferred_authenticator_id": 3, "locked": [] },
  "roles": ["ROLE_USER"],
  "organizations": [ { "uuid": "…", "name": "Acme", "roles": ["OWNER"], "primary": true, "personal": false } ],
  "preferences": { "language": "en", "theme": "dark", "timezone": "America/Chicago", "ciba_channel": "PUSH", "ciba_user_code_set": false },
  "favorite_apps": [ { "clientId": "conductor", "clientName": "Conductor", "iconUrl": "…", "homeUrl": "…", "customLabel": null, "order": 0 } ]
}
```
...
| `GET /api/user/tfa/enrol` | `{ qr, secret, issuer }` | `GET /accountconfig/edittfaapp` (a Thymeleaf fragment) |
...
| enrol SMS | `POST /api/user/tfa/sms/send` `{ mobile_number }` (`429` `throttled` with `wait_seconds`), `POST /api/user/tfa/sms/verify` `{ mobile_number, code, label }`, stepped up | `/accountconfig/sendmobilecode`, `/accountconfig/verifymobilecode` |
| enrol an app | `GET /api/user/tfa/enrol`, then `POST /api/user/tfa/app/verify` `{ code, label }`, stepped up | `/accountconfig/edittfaapp`, `/accountconfig/verifytfaapp` |
...
| favorites | `PUT /api/user/favorites` with the whole ordered list as `[{ client_id, custom_label, order }]`; the server answers the enriched entries from the client's own registration, so a `home_url` or `icon_url` the page sends is ignored and the favourites keep working as they do today | `POST /user/favorites/save` |
| preferences | `PATCH /api/user/preferences` unchanged in shape, its `400 { error }` becoming `422` with pointers | unchanged |
| delete the account | `POST /api/user/deletion` `{ email_confirmation }`, stepped up (`422` on `/email_confirmation`, `409` `sole_owner` with the team names); the server invalidates every session of the account before answering `{ next: "/login" }`, and the page drops its cache and navigates there | `POST /user/delete-account` |
...
- **ProfilePage** keeps its avatar card, its tab strip and its `account`
  ...
  - **Security** at `#security`: the password section (current password
    ...
    click), the locked-method notices with "Re-enrol now", the SMS risk
    ...
    **Passkeys**, its own section as today and never a row of the
    ...
    Regenerate; **Delete account** with the email confirmation labelled
  ...
  - **Preferences** at `#preferences`: language and theme as selects that
    write through on change, the same values the chrome's buttons write,
    so a change here is a change there and nothing is lost by leaving;
...
- **OrganizationsPage** at `/user/organizations`: Create an organization
  (name, labelled, while `organizations_enabled`), Join an organization
  (the invite code, labelled), then one card per membership (name,
  Personal, Primary, your role, Make primary, and only while `can_manage`
  the invite code with Regenerate) whose View button, Manage for a
  manager, sets the active organization and opens `/org-console`, so a
  plain member reaches the read-only console too; a `#<uuid>` in the URL
...
- **OrgConsolePage** on the issuer draws, beyond the shared record
  ...
  `AddressFields`, every field labelled), Convert to a team on a personal
...
- **IntegrationsPage**: Linked external accounts (provider icon, name,
  ...
  permission chips each with a 24px remove control labelled "Remove
...
- **InboxPage**: the same `NotificationRow` as the modal in a full-width
  ...
  all (behind a confirm) at the top, the per-row controls labelled "Mark
  ...
  on the chrome updated through the same `onUnreadDelta`.
...
| `InboxList` | `src/features/notifications/components/InboxList.jsx` | the row list the modal and the page both draw |
...
`setPreferred`, `remove`, `lastMethod`, `locked`, `reenrol`, `addPhone`,
...
| organizations | fourteen form posts each redirecting with a flash message; every organization's full console on one long page | one JSON per membership; the card list plus the shared console for the active organization |
...
modal, and two rows with one word and two destinations confuse. On the
...
| `GET /api/admin/roles` | `["ROLE_USER", "ROLE_ADMIN", …]` | `GET /admin/users/{id}/roles`, which the bulk dialog calls on the first selected user only to learn the catalogue |
...
| `GET /api/admin/insights` | `{ active_users: { daily, weekly, monthly, quarterly }, posture: { enabled_users, total_users, using_2fa, email_verified, phone_verified, admins, never_logged_in, with_local_auth, with_external_auth, with_linked_provider }, app_activity: [], penetration: [], app_pairs: [], growth: [], churn: { quiet_30, quiet_60, quiet_90 }, quiet_users: [], org_rollup: [] }` | `InsightsService.buildInsights` |
| `GET /api/admin/client-health` | `{ summary: { status, healthy, total }, clients: [{ client_id, client_name, description, base_url, healthy, status, response_time_ms, last_checked, error_message }], providers: [ the same ] }`; `healthy` is `null` for a client that cannot be probed | the client-health model and `/client-health/status` |
...
| `GET /api/admin/config/schema`, `/values`, `/base-defaults`, `/restart-status` | the issuer's own documents, unchanged in shape (decision 16); `restart-status` is `{ restart_required, last_modified_by, last_modified_time }`, read once when the stream connects, every change after riding the `admin` topic's `restart-required` event | `/admin/config/*`; `restart-status` polled by the footer every thirty seconds |
...
| rate limit | `POST /api/admin/rate-limit/{user_id}/unlock`, `/tfa-unlock`, `/ban`, `/unban` | `/admin/rate-limit/*` with `userId`; no page calls them |
| terms | `POST /api/admin/terms` `{ name, friendly_name, icon, version, type, is_public, display_order, content }` (`409` `unique` on `/name`), `PATCH /api/admin/terms/{name}` any of those, `DELETE /api/admin/terms/{name}`; each saved at once. The issuer's `/api/rules` carries a `terms` form: `name` is `$defs.slug` and `unique` globally, because it becomes the `/public/policies/<name>` and `/api/admin/terms/{name}` segment; `icon` matches `^[a-z0-9 -]{1,64}$` and is drawn only as a class attribute; the placeholders are a fixed list replaced by string substitution and never evaluated, so the editor can never reach a template engine | `/admin/terms/template/create`, `/update/{name}`, `/template/delete/{name}` form-encoded, batched behind the config editor's Save |
...
| configuration | `PUT /api/admin/config/values` (`422` with a pointer per failing path in place of the `400` text list), `POST /api/admin/config/restart` and `POST /api/admin/config/rotate-signing-key` → `{ kid }`, each stepped up and behind a `ConfirmModal` because one click must not restart the issuer or retire its signing key, `POST /api/admin/config/test-smtp` `{ smtp_id, recipient, config }` | `/admin/config/*` |
...
- **The admin pages** are one page per sidebar entry in
  ...
  - **Users**: the search field, the Status select (one value of Active
    ...
    the 2FA badge; the row actions as labelled buttons or one row menu,
  ...
  - **Activity**: three pages, one per sidebar row, no tab strip:
    ...
    and No), Sessions (its search bound to the navbar box over user and
    application, because the page has no other filter, the table with
    Authorized and Last active as two columns and Revoke behind a
    confirm); `Pager` under each; every table draws one date format, the
    absolute time in the cell and the relative time in its tooltip,
    because two formats on one screen read as two clocks; every table
    sits in a wrapper with `overflow-x: auto` and hides columns through
    the Columns group, so the page body never scrolls sideways.
...
- **The footer** draws only while the site's payload lists `footer`, and
  its heart only while `health` is listed too, coloured from `GET
  /api/health` on connect and from the `admin` topic's `health` event
...
| the bulk catalogue | the first selected user's roles call | `GET /api/admin/roles` |
...
| the footer | the client-health icon and a thirty-second restart poll on every page | the Client health page's summary line, the Dashboard and Configuration restart cards from the `admin` topic, the heart from the same topic while `footer` and `health` are both listed |
...
13. `/user/organizations` is a card list opening the shared console.
...
45. The footer draws only while `footer` is listed, its heart only while
    `health` is listed too, and the heart's state rides the `admin`
    topic's `health` event.
...
50. The admin tables carry their filters on the page; the Sessions page
    alone binds the navbar search.
