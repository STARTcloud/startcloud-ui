...
169. The sign-in page's switches are per client, never per site:
     ...
     `hide-remember-me` and `login-mode`, read by `GET /api/auth/methods`
     ...
     `sign_in_link_enabled`, `remember_me_enabled`, the `methods[]` rows
     ...
     that, never add; the hide is a page rule and not a refusal, the
