...
| `load()`                                  | `GET /api/user` through the API client's `auth: 'optional'` mode, which neither replays nor ends the session on a `401`: on `200` caches the display fields, applies `preferences.theme`, `preferences.pack`, `preferences.motion` and `preferences.language` to local storage as the branding contract requires on every adopt, and answers the profile; on `401` clears the cache and answers `null` (nobody was signed in); on `403` with `code: onboarding_required` caches the pending profile the body carries beside `code` and `next`, its display fields `name`, `email`, `picture`, `roles`, `organizations` and `has_local_auth`, answers it as signed-in-pending and navigates in-router to the answer's `next` |
| `reload()`                                | `load()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
