...
sections, tree? }` with one section, Account: Profile (`/user/profile`),
...
admin feature keeps its three entries, Organizations and users,
Configuration and System at `/admin`, `/admin/config` and
`/admin/system`, each drawn only while the adapter carries
`organizationsWithUsers`, `config` or `storage`, its Configuration entry
the same tree over `status.config` while the list names more than one
file; on the issuer the router mounts the identity feature's column in
