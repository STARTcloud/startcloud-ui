...
- **The spec supersedes this contract.** Where a clause here and a
  ...
  changes; the specifications are RFC 6749, RFC 6750, RFC 7009, RFC
  8628, RFC 9126, RFC 9207, RFC 9396, RFC 9449, RFC 9457, RFC 9700, RFC
  9110, OpenID Connect Core 1.0, Discovery 1.0, Back-Channel Logout 1.0,
  Front-Channel Logout 1.0, RP-Initiated Logout 1.0, Session Management
  1.0, CIBA Core 1.0 and the FAPI 2.0 Security Profile, and every clause
  that restates one of them names the section, because a contract that
...
| `load()`                                  | `GET /api/user` through the API client's `auth: 'optional'` mode, which neither replays nor ends the session on a `401`: on `200` caches the display fields, applies `preferences.theme`, `preferences.pack`, `preferences.motion` and `preferences.language` to local storage as the branding contract requires on every adopt, and answers the display fields with the account's `preferences` beside them as `preferred_theme`, `preferred_pack`, `preferred_motion` and `preferred_language`, never cached, so the chrome adopts the account's choices the moment the profile answers and not on the next reload, a guest-only account's neither mirrored nor answered, the browser's own standing; on `401` clears the cache and answers `null` (nobody was signed in); on `403` with `code: onboarding_required` caches the pending profile the body carries beside `code` and `next`, its display fields `name`, `email`, `picture`, `roles`, `organizations` and `has_local_auth`, answers it as signed-in-pending and navigates in-router to the answer's `next` |
| `reload()`                                | `load()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `endSession()`                            | drops every storage key but `theme`, `pack`, `packs`, `motion` and `language` and ends the session on the bus                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `refresh()`                               | `load()`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `claims()`                                | memoized `GET /api/userinfo/claims`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `savePreferences(patch)`                  | `PATCH /api/user/preferences`, then `preferences.theme`, `preferences.pack`, `preferences.motion` and `preferences.language` of the answer applied to local storage as `load()` does; the cache holds the display fields alone and is never widened, so nothing in it is updated                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `signOut()`                               | `POST /user/logout` with the CSRF header, answering `200 { "next": "/connect/logout/frontchannel" }` while relying parties registered front-channel URIs and `{ "next": "/login?logout" }` otherwise; the provider drops every storage key but `theme`, `pack`, `packs`, `motion` and `language` and sets `window.location` to `next`                                                                                                                                                                                                                                                                                                                                                                                       |
...
replacing the `login_method_pref` cookie the server read), `theme`,
`pack`, `packs`, `motion` and `language` (the chrome's own keys, `pack` the
chosen look's name, `packs` the host's offered list cached for the
pre-paint script and `motion` the person's reduced-motion switch),
...
are empty or absent. The payload's own members are `snake_case`
...
| `brand.pack`                    | `{ name, css }` while the site's `theme_id` names a pack, `name` the pack and `css` the stylesheet URL on the serving origin, no query string and no version in it, the same values the site's `index.html` is stamped with; present on every site, because every site names a pack                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | the shell reads one member for a pack on every UI backend and calls no branding route; the issuer stamps the same two values into `index.html` so the pack paints with the first frame and the shell appends nothing                                                                                                                                                                                                                                                                                                                                                                             |
| `brand.packs`                   | `[{ name, css, label }]` from `sites.<id>.ui.packs`, a list of bare pack names of the shared build in the site's order, each `css` built as `brand.pack.css` is and `label` the site's display name for it; absent while the list is absent or empty                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | the shell draws its Look picker from this list alone and constructs no stylesheet URL; a site that offers none shows no picker, so a white-label site never shows a sibling's look; the person's choice is the `pack` preference, applied by the pre-paint script from the cache, so the stamp stays the site's                                                                                                                                                                                                                                                                                  |
...
| `password_reset_enabled`, `sign_in_link_enabled`   | whether the page draws the Forgot password link and whether the emailed sign-in link is offered as a mode; both open unless the answer says false, registration alone closed until its member opens it (`signInAffordances`); the site decides what exists on the page, and the client whose authorize request is parked in the session may only hide or reorder inside that, never add, through its `clients.<id>.client.hide-password-reset` and `hide-sign-in-link`, the site's answer as it is while none is parked; `remember_me_enabled` and `local_registration_enabled` are narrowed the same way by `hide-remember-me` and `hide-registration`, `mark_enabled` is the parked client's `show-mark` when set, else the site's `ui.show_mark` when set, else the issuer's `application.ui.show_mark`, each tier overriding the one above in either direction, the page then drawing the site's own wordmark, `logo.svg` beside `brand.logo_url`, above its heading and nothing else changing, `hide-password` and `hide-passkey` drop those methods from `methods[]`, and `login-mode` names the mode shown first, moved to the other mode when it points at a method the client hides; because a client such as BoxVault's downloads face wants a plain sign-in for its own flow while every other client on the same site keeps the full page | `AuthMethodsApiController`, the parked `/oauth2/authorize` request's `client_id` from the request cache                      |
| `remember_me_enabled`                              | false while the parked client's `hide-remember-me` is true, which hides the Keep me logged in box at all times, false shows it; the page draws the box only while true and sends no `remember-me` otherwise                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `AuthMethodsApiController`, the parked client's `hide-remember-me`                                                           |
| `mark_enabled`                                     | the parked client's `show-mark` when that key is set, true or false; else the site's own `sites.<id>.ui.show_mark` when set, true or false; else the issuer's `application.ui.show_mark`, false by default; each tier overrides the one above in either direction when set and inherits it when unset, so an issuer may want the mark on every site but one and a site on every flow but one; the page then draws the site's own wordmark, `logo.svg` in the folder of `brand.logo_url` of `/api/status`, above its sign-in heading and nothing else changes, the logo with text there because the mark alone above the form seemed small and the header already draws the mark; sites for sure should have this, not just client apps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `AuthMethodsApiController`, the parked client's `show-mark`, else the site's `ui.show_mark`, else `application.ui.show_mark` |
| `cancel`                                           | whether a client's authorization request is parked in the session, so the page draws Cancel only when there is something to cancel; a Cancel on a plain visit would loop `/auth-cancel` → `/` → `/login`; a client whose `clients.<id>.client.hide-cancel` is true hides the link for its own flow, the route behind it staying open, like every other per-client switch                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `requestCache.getRequest` non-null                                                                                           |
...
new version on every publish (decision 161), so the words a person
...
beside the set decision 131 names, so a punctuation fix saved to the
...
- **TermsPage**: classic mode is the title, "Review and accept these
  ...
  in the policy (decision 136); every field the document references drawn
  ...
  variants, the way the old issuer's MLA page drew its assurance card and
  its acknowledgment row, because the sanitizer keeps exactly those
  ...
  on every site and never plain prose (decision 131).
...
- **ConsentPage**: "Authorization Request", "{{client}} is requesting
  ...
  when `scopes` is empty and an authorization detail is up for approval
  (decision 165); "This application also requests
...
| `/user/profile` (and `/profile`) | the one shared ProfilePage over the issuer's `account` adapter: the sidebar's Profile row is this page (`/user/profile`) and its child rows, built from the sections the adapter carries, Security (`/user/profile/security`), Preferences (`/user/profile/preferences`), Favorites (`/user/profile/favorites`) and Sessions (`/user/profile/sessions`), are each a deep link into the one page, no tab strip on the issuer (decision 109); the page's Organizations and Service accounts sections are not drawn on the issuer because its adapter carries neither `organizations` nor `serviceAccounts`, the sidebar's Organizations row being the memberships' destination | `cookie`                                                       | `user/profile.html`, `UserController`; `/.well-known/change-password` redirects to `/user/profile/security`, the page the W3C well-known URL must lead to, in place of the template's anchor |
| `/user/organizations`            | OrganizationsPage: the memberships under the one view toggle, list or cards, Create a team (a name is required) and a "Find an organization" link to the directory, no code form; a row or card opens the console for that organization                                                                                                                                                                                                                                                                                                                                                                                                                                      | `cookie`, `org-console`                                        | `user/organizations.html`, `OrganizationController`; the join-by-code form goes                                                                                                              |
| `/organizations/discover`        | the shared DiscoveryPage of the pages contract over the issuer's `organizations` adapter: every organization open to discovery with its `access_mode`, `invite`, `request` or `private`, Request to join on a `request` one, no action on the others, because joining is the organization's choice of door and never a code a person types; the page and the Discover row draw on the `discover` token alone, as the pages and navbar contracts gate them, so the issuer advertises it while its directory is on                                                                                                                                                             | `cookie`, `discover`                                           | new; BoxVault's `GET /api/organizations/discover`                                                                                                                                            |
| `/org-console`                   | the shared OrgConsolePage over the active organization, grown by the issuer's fields; its crumbs are the Organizations row's child, Account, Organizations, then the active organization's name as the last crumb, plain text, the route's `crumbParent` being `/user/organizations` (decision 130)                                                                                                                                                                                                                                                                                                                                                                          | `cookie`, `org-console`; `invitations` for its Invitations tab | the per-org sections of `user/organizations.html`                                                                                                                                            |
...
| `GET /api/organization/{org}/requests`             | `[{ id, user: { id, name, email }, message, created_at }]`, the pending join requests of an organization, `can_manage` alone, the shape BoxVault answers on the same path, read by the console's Join requests tab through the adapter's `requests`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | new; BoxVault's `organizationRequests`                          |
| `GET /api/user/integrations/providers/{id}/status` | `{ status: "valid" \| "revoked" \| "unknown" }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | `GET /user/integrations/api/provider-status`                    |
| `GET /api/notifications?page&size&unread_only`     | `{ items, page, size, total, total_pages }` of the hub contract's rows in `snake_case`, `read_at` `null` while unread in place of today's `isRead` and `readAt` pair, `created_at`, the one row shape the shared `NotificationRow` draws on BoxVault, the catalog and the issuer alike; and a `navigate` the page follows when it is an `https://` URL or a same-origin path, because the issuer's own producers write `/user/profile` and `/user/integrations` and a path cannot carry a scheme; the fields the shared `NotificationRow` reads on every UI backend                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | unchanged                                                       |
...
| preferences                | `PATCH /api/user/preferences` with `language`, `theme`, `pack`, `motion`, `timezone`, `region`, `ciba_channel` and `ciba_user_code`; `pack` a bare pack name the site offers in `brand.packs` or `null` to follow the site's own, `422` `enum` at `/pack` for any other; `region` the person's legal region, a two-letter ISO 3166-1 code or one of `EU`, `EEA`, `UK`, `null` clearing it, the value the terms page's region button writes and the terms and policy copies resolve to first (`422` `enum` on `/region` otherwise); `ciba_user_code` a string that sets the approval PIN and `null` that clears it, never read back, `ciba_user_code_set` being the read's word for it; its `400 { error }` becoming `422` with pointers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | unchanged                                                                                                        |
...
| inbox                      | the hub's `POST /api/notifications/{id}/read`, `POST /api/notifications/read-all` and `DELETE /api/notifications/{id}` unchanged; `POST /api/notifications/{id}/unread` puts one row back to unread, clearing `read_at`, answering `204` and pushing the hub's `unread-count` event the way `/read` does, `404` for a row that is not the caller's (decision 151); `DELETE /api/notifications` deletes every notification of the caller and answers `204`, behind the page's Delete all confirm                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `DELETE /api/notifications/{id}` and `DELETE /api/notifications` both answered by `NotificationController`; done |
...
- **ProfilePage** is the pages contract's one profile page, the same
  ...
  there (decision 143); it keeps its `account` adapter; on the
  ...
  its section under the page heading (decision 109); the page draws a
  ...
  - **Profile** at `/user/profile`: First name, Last name, middle name, salutation (the six
    ...
    script origin is already in the policy (decision 136).
...
- **OrgConsolePage** on the issuer draws, beyond the shared record
  ...
  organizations template gates on today; a `guest` is read-only below
  ...
  organization it shows, never by a title (decision 130).
...
| `/admin/email-templates`                                                                    | Messaging › Email templates: the nine kinds as cards, each with its copies per site and locale, create, edit, preview, history, publish, delete (decision 164)                                                                                                                                                                     | the same                            | the `email.*` keys of `messages.properties` and their `<siteId>.email.*` overrides, edited in the jar                                                                                                                                      |
...
and last, as the group's `tree` of decision 68 while `integrations` is
...
shared configuration page (decision 16); the row alone while the list
names one file, and while it names more than one the group's `tree` in
the row's place, the shared `useConfigTree` of `src/hooks`, fed the admin
...
| email templates           | `POST /api/admin/email-templates` `{ kind, site, locale, version, subject, body }` (`422` `enum` at `/kind`, `409` `unique` at `/site` when a copy of the kind already carries the same site and locale), creating the copy at revision 1 of its `version`; `PATCH /api/admin/email-templates/{kind}` any of `subject` and `body`, the Save act of decision 161: revision r+1 under the copy's current version, the live text changed, and a `version` member refused `422` `readOnly` at `/version`; `DELETE /api/admin/email-templates/{kind}`, refused `409` `conflict` with `code` `default_copy` for the default copy of a kind, which is seeded at first boot and never deleted; the two addressing the default copy unless `?site=&locale=` names one, the site's default copy on `site` alone; a `{n}` beyond the kind's argument list refused `422` rule `format` at `/subject` or `/body` with `params: { argument: n }`, because a `MessageFormat` given fewer arguments than the text names leaves the brace in the mail; the arguments are substituted by the server, the body's HTML-escaped and the subject's plain, and the template is never sanitized, being the administrator's (decision 164)                                                                                                                                                                                                                                                                                                                                                              | `<siteId>.email.<kind>.subject` and `.body` keys in `messages.properties`, shipped in the jar                                     |
| publish an email template | `POST /api/admin/email-templates/{kind}/publish` `{ version, subject?, body? }`, the default copy unless `?site=&locale=` names one, the Publish act of decision 161: `version` required and different from the copy's current one (`409` `unique` at `/version` with `params.scope` the copy), `subject` and `body` optional and the current text when absent; it writes revision 1 of the new version and makes it the live text; nobody is notified, a template having no acceptors (decision 164)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | new                                                                                                                               |
| email templates bulk      | `POST /api/admin/email-templates/bulk` `{ action: "delete", ids: [] }`, `ids` the copies' ids, answering `{ processed, skipped, errors: [{ id, code }] }`, an unknown id skipped with `not_found`, a default copy with `default_copy`, `422` `enum` at `/action`, `422` `required` at `/ids`; stepped up (decision 164)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | new                                                                                                                               |
...
- **The admin pages** are one page per sidebar entry in
  ...
  - **User**: the record page at `/admin/users/:id`, reading
    ...
    preferences section with the record's language, theme and motion as
    fields saved with the rest over the preferences `PATCH`, never the viewer's
  ...
  - **Terms**: a `SectionHeading` over the cards, its title Terms, Create
    ...
    read, and notifies every prior acceptor of that copy as today; a
    ...
    a new version then (decision 161); Add a copy on a card opening the
  ...
  - **Configuration**: the shared `AdminConfig` of the config contract
    ...
    nodes on more, as in-router links (decisions 16 and 122).
...
16. The configuration editor is the shared editor of the config contract
    ...
    UI backend whose `status.config` names a file, one file per route,
    `/admin/config/<name>`, each a child node of the Configuration tree
    named by those names; no stopgap was built.
...
21. `features` is exactly the set of tokens the contracts gate on, `footer`
    ...
    is the build-info version; `ticket.fallback_customer_id` is the site's
...
52. The status payload's members are `snake_case`, like every other body
    of the estate.
...
65. The cookie provider's cache holds the display fields alone, `name`,
    ...
    `savePreferences` reapplies `theme`, `pack`, `motion` and `language` to
    local storage and updates nothing in the cache.
...
72. The generator writes one file per pack,
    `public/themes/<pack>/<pack>.css`, and nothing beside it: no digest
    file, no `?v=` anywhere, the pack served `no-cache` with an ETag like
    every other file of the build.
...
78. Notification rows are `snake_case` like every other answer, `read_at`
    and `created_at`, on every UI backend; the inbox read answers
    decision 19's paged shape.
...
104. The push worker is `/notification-sw.js` at the origin root, scope
     `/`, registered by the shell with the app name and the build's
     version in its query, the two headers sent on that one path.
...
109. A sidebar row may carry `children`, a list of rows in the row shape;
     ...
     issuer's profile draws no tab strip; a `backend` UI backend draws
     the same page and the same row shape at `/profile`, its children the
     sections its own adapter carries.
...
119. Every identity page that draws a table or a card list publishes one
     ...
     rule: Client health Status, Provider health Status, Users Status, 2FA, Customer ID
...
122. The configuration editor's files are never tabs on the page: the
     page draws one file per route, `/admin/config/<name>`, and the bare
     `/admin/config` redirects to the first name's route so a deep link
     still lands, that file's
     sections under the page heading with no tab strip (decision 105);
     while `status.config` names more than one file the column's
     Configuration entry is the group's `tree` of the navbar contract in
     the row's place, one Configuration node with no `to` of its own,
     folding on click and never navigating, whose
     children are one node per name in list order, labelled by the
     schema's root `title` once `GET /api/config/<name>/schema` has
     answered and by the name when it does not, each a deep link to
     `/admin/config/<name>`, the schemas fetched once through the admin
     adapter and shared with the page; while it names one file the plain
     Configuration row alone; the identity feature's column and the
     shared admin feature's entries carry the same tree through the one
     `useConfigTree` hook; the breadcrumb on a child route reads Admin,
     Configuration, then the child's label, Configuration plain text;
     because a file is a place
     the column names and the column is the one navigation, a tab
     strip beside it is a second one, two rows that open one page
     confuse, and a node that only folds is a group, not a page.
123. An application the estate controls is never an integration; an
     integration is a third-party service connected through the issuer.
     The Integrations page splits four ways: the linked external
     accounts move to the profile's Security page as its Linked accounts
     section, read from `GET /api/user/linked-accounts`, the link and
     unlink calls unchanged; the accepted terms and policies are the
     Account page `/user/terms`, read from `GET /api/user/terms`, its
     sidebar row Terms and policies while `policies` is advertised; the
     connected applications are the Account page `/user/applications`,
     read from `GET /api/user/applications`, its sidebar row
     Applications always on the issuer; `/user/integrations` keeps the
     third-party services alone, read from `GET /api/user/integrations`
     as `{ services: [{ id, name, icon_url, status, connected_at,
settings_url }] }`, its page and its Integrations entry drawn only
     while `services` is answered, the entry the Account group's `tree`
     of decision 68 and so the last of the column's Account entries,
     after Inbox, because a row that exists only once a read has
     answered is a tree node and a tree draws after the sections, so an
     estate that connects none draws no row; every new path is under
     `/user` and no first segment is reserved for it.
124. Joining an organization on the issuer is directory-driven, BoxVault's
     shape: the shared DiscoveryPage at `/organizations/discover` lists
     the organizations open to discovery with their access mode,
     `invite`, `request` or `private`, from
     `GET /api/organizations/discover`; a `request` organization carries
     Request to join, `POST /api/organization/{org}/requests { message }`,
     which notifies its admins and lands in the console's Join requests
     tab over `GET …/requests`, `POST …/requests/{id}/approve
{ assigned_role }` and `POST …/requests/{id}/deny`; an `invite`
     organization is joined from the invitation mail's link alone; a
     `private` one shows no action; the issuer's adapter carries
     `discover` and `requests`; the Organizations page keeps Create a
     team, a name required, gains a "Find an organization" link to the
     directory and drops the join-by-code form, and
     `POST /api/user/organizations/join` retires with it, because a code
     a person types is a secret that leaks and a door the organization
     never chose. The issuer advertises `discover` in its `features`
     while its directory is on, as BoxVault does, and
     `/organizations/discover` is gated on `cookie` and `discover`,
     never on `org-console` alone, because the pages contract draws the
     page and the navbar contract's token table draws the Discover row on
     that token and a surface gated on a token the payload never answers
     would never draw. `access_mode` is one word on every UI backend,
     `snake_case` with exactly `invite`, `request` and `private` on the
     record, the discover list and the access-mode write; BoxVault's
     `accessMode`, `invite_only` and `request_to_join` converge on it in
     its round, because the shared page reads one word per door.
125. The All organizations page carries Edit over the whole record:
     `PATCH /api/admin/organizations/{id}` over `name`, `email`,
     `website_url`, `logo_url`, `description`, `locale`, `timezone`,
     `telephone`, `address`, `access_mode`, `default_role` and
     `customer_id`, answered `200` with the record, `422` with pointers
     and `409 unique` on the name, the row's Edit opening the form dialog
     of decision 121 over those fields, prefilled from the list row,
     which carries them; because an operator who can delete an
     organization but not correct its record is sent to the owner for
     every typo.
126. The Terms page's Preview never leaves the page: it opens a list
     dialog of decision 121 drawing the template as the person would see
     it, from the `content` markdown `GET /api/admin/terms` already
     carries, through the shared `MarkdownArticle` under the template's
     title, version and type badge, for every template public or not; a
     public card carries a second action, "Open public page", linking to
     `/public/policies/<name>` in a new tab; because
     `GET /api/policies/{name}` refuses a non-public template with `404`
     by design, and an admin must read a client's terms before assigning
     them.
127. Health splits by kind: the Health section's rows are Service usage,
     Insights, Client health and Provider health; Client health at
     ...
     the card's name links, for an admin, to the config item deep link
     `/admin/config/clients#<client_id>` or
     `/admin/config/providers#<provider_id>` of the config contract's
     section 7, and the search table's `application` and
     `identity-provider` rows link an admin to the same deep links, a
     member keeping `/user/applications` and `/user/profile/security`
     (decision 123); because a client and a provider are two records in
     two files with two owners, one page over both read as one list of
     mixed kinds, and the entry a failing probe or a search hit names is
     the entry an operator opens to fix it.
128. The configuration tree's hook is the shared `useConfigTree` of
     `src/hooks`, handed the admin adapter's `config` by each feature's
     sidebar export, the identity feature's `sidebar(status, account,
admin)` taking the adapter the router builds as the shared admin
     feature's does; the tree loads the children of the node the current
     route descends from on mount, so a deep link to
     `/admin/config/<name>` crumbs Admin, Configuration, then the file's
     `title` before the node was ever expanded; and a section row's
     `children` fold like a tree node's under the same
     `sidebar_open_<group>`, open while a child is the current route;
     because the pages contract lets a feature import another only
     through the collections registry and `features/deploy`, and a crumb
     that reads Admin alone on a file's own route tells the person less
     than the column beside it.
129. The configuration page's heading is the file's root `title`, drawn
     in the pages contract's `PageHeader` shape with Update as its
     action, and the shared admin page's own heading is not drawn above
     it, because the crumb reads Admin, Configuration, then the file, and
     a heading that repeats the column is a second name for one page;
     `schemaVersion` is never a section: it draws as a muted line under
     that heading, `configManager.schemaVersion` ("Schema version 1"),
     and a section whose only leaf is `schemaVersion` is not drawn,
     because the version is a fact about the file and not a setting an
     operator changes; the config contract is the rule's home.
130. A page reached from a sidebar row but living at its own path crumbs
     as that row's child: the group, the row linking to its own page,
     then the page's own name last as plain text, the route table naming
     the row's path as the route's `crumbParent` and the name the page
     carries; the organization console at `/org-console`, opened by
     Manage on `/user/organizations`, reads Account, Organizations, then
     the active organization's name from the active membership, and the
     page's title only while no organization is active; because the
     crumb is where the person came from and a page named by its data is
     named by that data, never by a title that repeats the column.
131. The sanitizer keeps `class` only from a named set, `callout`,
     `callout-lock` and `callout-doc` on a block element and `tos-blank`
     on the live-fill span, every other class token dropped, and the one
     stylesheet draws a `callout` and a flowing document's `blockquote`
     as a card on the auth tokens with the lock or the document glyph
     before its first line in both variants; because a class is a hook
     into the app's own stylesheet, a document must reach the cards and
     the blanks the contract names and nothing else, and the old issuer's
     MLA page drew its assurance card and its acknowledgment row that way.
132. The files Vite creates are never hashed: every entry, chunk,
     stylesheet and asset keeps its fixed name (`assets/<name>.js`,
     `assets/<name>.css`), and no build step, plugin, server or contract
     may add a content hash to a file name, ever, and no query string
     ever carries a version or a hash either, a pack's stylesheet and the
     `theme_css` the branding endpoint answers included; caching is the
     server's job through `no-cache` and an ETag per file. Nothing under the
     served folder is immutable: every file, `/assets/` included, is
     served `Cache-Control: no-cache` with an ETag, a conditional request
     per file and the bytes only when they changed, never `immutable`,
     and `index.html` and `/` `no-store`; because a fixed name cached for
     a year pins one deploy's chunk against another's, and a lazy chunk
     fetched fresh then binds to the wrong export table.
...
134. Every page of the shared UI on every UI backend frames its body by
     ...
     `table_prefs_*` object (decision 117), the Dashboard's under
     `table_prefs_admin_dashboard`; a list or a table draws glass, a
     `SectionHeading` and the rows or the table straight on the ground
     with no card body; stat tiles, maps and notices are cards. On the
     issuer the profile's Security and Preferences, the Create-a-team
     form, the organization console's record and invitation forms and
     the configuration editor's sections are cards; Favorites, Sessions,
     the inbox, the organizations memberships, Applications, Terms and
     policies, Integrations, the console's members, invitations and join
     requests, the discovery directory and every admin table and card
     list are glass; the Dashboard's tiles, map and restart notice are
     cards and its two recent tables glass; because a card frames fields
     a person fills, a list reads better without one, and one rule keeps
     every page of every UI backend alike.
135. A tree hook's answer may carry a `labelKey`, `{ nodes, menu?,
labelKey? }`, drawn as a section heading above the tree in the same
     style as a section's, the navbar contract's Export bullet naming
     it; `useConfigTree` takes the key as its second argument and
     answers it beside its nodes, the issuer's Configuration tree of
     decision 122 and the shared admin feature's both passing
     `admin.sidebar.system`, so the column reads System above
...
136. The terms page's address group is the shared `AddressFields` block
     ...
     state, city, postal code, the key read from the same session-gated
     `GET /api/config/places`, every part prefilled from the listed
     field's `value` and posted under that field's `param`; and every
     terms page's wide column is 1120px at most, the viewport minus
     the page padding below that; because the person types the same
     address here as on the profile and gets the same help, the key is
...
147. The All organizations bulk route grows:
     ...
     `set_default_role` (the member `default_role`, `MEMBER`, `ADMIN` or
     `GUEST`, `422` `enum` at `/default_role`) and
     `regenerate_invite_code`; a
...
151. The Inbox bulk bar gains Mark as unread beside Mark as read and
     ...
     answering `204`, clearing `read_at` and pushing the hub's
...
159. Every version of a copy is kept, the revision rows and the two acts
     ...
     the publish of decision 161 writes the new version and notifies the
     acceptors as today; because a person asked to accept a
...
161. End users are prompted on the version change alone, and under every
     version the server keeps internal revisions: a copy's `version` is
     the public version people accept, free text up to 50 characters, a
     date or a semver alike, only equality mattering; under it the server
     numbers revisions itself from 1, `revision`, a revision row
     `{ version, revision, content, published_by, published_at }`, the
     `terms_revision` store of decision 159 gaining `revision` and its
     unique key becoming copy, version and revision. Two admin acts on a
     copy. Save, `PATCH /api/admin/terms/{name}` (`?region=` for a copy)
     with `content`, writes revision r+1 under the current version,
     changes the live text and re-prompts nobody; a `version` member on
     the PATCH is refused `422` `readOnly` at `/version`, because the
     version changes only by publishing; `friendly_name`, `icon`, `type`,
     `is_public` and `regions` stay PATCH members as they are. Publish,
     `POST /api/admin/terms/{name}/publish` (`?region=` for a copy) with
     `{ version, content }`, `version` required and different from the
     current one (`409` `unique` at `/version` with `params.scope` the
     copy when it equals an existing version of that copy), `content`
     optional and the current text when absent, writes revision 1 of the
     new version, makes it the live text and notifies prior acceptors as
     the version bump does today. `POST /api/admin/terms` creates a copy
     at revision 1 of its version. `GET /api/admin/terms/{name}/history?region=`
     answers `{ name, region, versions: [{ version, revisions: [{ revision, published_by, published_at, current }] }] }`,
     newest first, `current` on the live one;
     `GET /api/admin/terms/{name}/history/{version}/{revision}?region=`
     answers `{ version, revision, published_by, published_at, content }`,
     raw markdown; the admin's per-document read gains `revision` beside
     `version` on each copy. `GET /api/auth/terms` `versions` stays a list
     of versions, never revisions, newest first;
     `GET /api/auth/terms/versions/{version}` answers the latest revision
     of that version; `changes_html` compares the current text against
     the latest revision of the version the person accepted, so a
     punctuation fix made after their acceptance never shows as a change;
     the acceptance record gains `revision`, the revision live when they
     accepted, for the audit alone and exposed on no read;
     `GET /api/user/terms` is unchanged. On the Terms page the copy's edit
     dialog has two actions, Save, the default, "Save as a revision of
     {{version}}", and Publish, which asks for the new version string in
     a small form dialog before it sends; a History action on the copy
     opens a list dialog of the versions with their revisions beneath,
     each revision openable read-only; the card shows `version` and
     `r{{revision}}` beside it. Every revision keeps who and when, so a
     lawyer who saved a real change as a revision by mistake is caught by
     the history and the admin publishes it as a new version then;
     because lawyers publish a new terms text and people accept it, a
     week later a punctuation typo is fixed and that must not re-prompt
...
163. The organization role vocabulary gains a fourth role, `guest`, beside
     `owner`, `admin` and `member`: a read-only membership below member,
     carried in the membership, the `organizations` claim and SCIM the way
...
164. Email templates are documents of the issuer the way terms are, never
     settings of the config engine: one document per kind, the nine kinds
     `account_verification`, `password_reset`, `magic_login`,
     `onboarding_verification`, `org_invite`, `email_change`,
     `account_event`, `ciba_approval` and `admin_new_registration`, fixed
     by the server and never created or deleted; each document holds
     copies, a copy keyed by `site`, a site id or empty for the default
     copy, and `locale`, a BCP 47 tag or empty for the site's default, two
     copies of one kind never sharing a site and a locale, refused `409`
     `unique` at `/site`; a copy's content is `{ subject, body }`, the body
     HTML carrying the kind's `{n}` arguments, and a copy carries a
     `version` and revisions under it exactly as decision 161 fixes them
     for a terms copy: `POST /api/admin/email-templates` creates a copy at
     revision 1 of its version; Save,
     `PATCH /api/admin/email-templates/{kind}?site=&locale=` with `subject`
     or `body`, writes revision r+1 under the current version, and a
     `version` member is refused `422` `readOnly` at `/version`; Publish,
     `POST /api/admin/email-templates/{kind}/publish?site=&locale=`
     `{ version, subject?, body? }`, writes revision 1 of a new version,
     `409` `unique` at `/version` when the copy has had it, and notifies
     nobody, a template having no acceptors;
     `GET /api/admin/email-templates/{kind}/history?site=&locale=` and
     `GET /api/admin/email-templates/{kind}/history/{version}/{revision}?site=&locale=`
     answer the history in decision 161's shapes with `subject` and `body`
     in place of `content`; `DELETE /api/admin/email-templates/{kind}?site=&locale=`
     removes a copy other than the default copy, refused `409` `conflict`
     with `code` `default_copy` for the default copy, and
     `POST /api/admin/email-templates/bulk { action: "delete", ids: [] }`
     removes many, a default copy skipped with `default_copy`, stepped up;
     every copy-addressing route takes the default copy when neither
     parameter is given and the site's default copy on `site` alone;
     `GET /api/admin/email-templates` answers every kind,
     `[{ kind, copies: [{ id, site, locale, version, revision, subject, body, created_by, updated_at }] }]`,
     `copies` never empty, every kind carrying at least its default copy; and
     `GET /api/admin/email-templates/arguments` answers
     `[{ kind, arguments: [{ index, name, description }] }]`, the fixed
     list per kind from the server's send methods, one list for the
     subject and the body alike (the `org_invite` subject moves to `{1}`
     for it), the body's values HTML-escaped, the subject's plain, the
     template itself never sanitized, and a `{n}` beyond the kind's list
     refused `422` `format` at `/subject` or `/body` with
     `params: { argument: n }`, the way `GET /api/admin/terms/placeholders`
     serves the terms placeholders. The generic text of each kind is its
     default copy, seeded: shipped in the jar as one seed file per kind,
     `email-templates/<kind>.json` carrying `{ kind, subject, body }`, and
     written to the database once, at the first boot that finds no
     default copy of the kind, at version `1.0` revision 1 with
     `created_by` and `published_by` `seed`, never overwritten by a later
     boot, editable like any copy and never deletable; no site key names
     kinds, and a send resolves the site's copy in the person's locale,
     then the site's default copy, then the default copy in the locale,
     then the default copy, and nothing else, so a fresh install sends
     mail with no configuration and `messages.properties` carries no
     email text, neither a brand's `<siteId>.email.*` key nor a bare
     `email.*` key. The shared UI draws the
     documents at `/admin/email-templates`, the Email templates row of a
     Messaging group beside Legal in the operator's column, through the
     terms page and dialogs: one card per kind with a row per copy naming
     its site and locale, the default copy's row reading "Every site" and
     a site's default locale "Every language", `version` and
     `r{{revision}}` beside it; the copy dialog of decision 121 with the
     subject field, the body in a monospace textarea and beside it the
     preview drawing the HTML in a sandboxed `iframe`, `sandbox` with no
     token and `srcdoc` the body with each `{n}` replaced by its
     argument's name, so a template's script or link runs nowhere; the
     argument chips above the textarea from the arguments route, each
     chip inserting its `{n}` at the caret; Save as a revision, Publish
     asking the new version, History and Delete as the terms copy has
     them; and the page binding the navbar search with a query over the
     cards by kind and a Site group narrowing the cards client-side; the
     keys `admin.sidebar.messaging` and `admin.emailTemplates.*`,
     placeholders the UI may rename. Because a template is content with
     revisions and per-locale copies, which is the terms shape, and not a
     setting at a pointer: the config engine gives one value per pointer,
     a `422` pointer and a restart flag, and no revisions and no
     per-locale copy, and a brand's HTML inside the server's jar makes
     every wording change a deploy.
165. The consent screen shows every scope the request named:
     `GET /api/auth/consent` answers `scopes`, the requested scopes not
     yet granted, and `granted_scopes`, the requested scopes already on
     file for this person and client, in the same row shape; the page
     draws the granted ones as locked, checked, read-only rows under
     "Already allowed" with a "Manage permissions" link to
     `/user/applications`, where `DELETE /api/user/integrations/apps/{client_id}/scopes/{scope}`
     and the app's Revoke live, because the server merges an approval into
     the stored consent and never narrows it from this screen; Approve is
     refused with the inline line only while `scopes` is non-empty and
     none of its rows is checked, and a consent whose `scopes` is empty
     approves the authorization details alone, because a RAR detail is
     owed consent on every request while the scopes were granted once,
...
166. The profile page's `account` adapter carries `mutability`, the SCIM
     ...
     answers and the Preferences the record's language, theme, motion, time
     zone and region; a section or card whose read the UI backend lacks is
...
167. The identity provider adds `GET /api/admin/users/{id}`, session or
     ...
     its preferences (language, theme, pack, motion, timezone, region,
     ...
     local password.
168. A person may choose the look of an application, the theme pack, the
     way they choose the variant: `PATCH /api/user/preferences` gains
     `pack`, a bare pack name the site offers or `null` to follow the
     site's own, refused `422` `enum` at `/pack` otherwise and
     `403 guest_only` for a guest-only account; the member is read back as
     `preferences.pack` on `GET /api/user`, on `GET /api/user/preferences`
     ...
     a `no-store` page buys nothing; because the variant is the person's
     already, a white-label site must never show a sibling's look, and the
     pack is a name of the one build every host serves.
169. The sign-in page's hide switches are per client, never per site:
     `clients.<id>.client.hide-registration`, `hide-password-reset`,
     `hide-sign-in-link`, `hide-password`, `hide-passkey`,
     `hide-remember-me`, `hide-cancel`, `show-mark` and `login-mode`, read by `GET /api/auth/methods`
     from the client whose authorize request is parked in the session and
     answered as `local_registration_enabled`, `password_reset_enabled`,
     `sign_in_link_enabled`, `remember_me_enabled`, `mark_enabled`, `cancel`, the `methods[]` rows
     and `login_mode`, the site's own answer while none is parked; the
     site decides what exists on the page through its own keys
     (`registration.self_registration_enabled`, `passkeys.enabled`,
     `ui.default_login_method`, `ui.show_mark`, `tos-names`, a provider's
     `allowed-sites` and `allowed-networks`) and a client may only hide or reorder inside
     that, never add; the one switch a client may set either way is the
     site's mark above the heading: `application.ui.show_mark` is the
     issuer's default, a site's `ui.show_mark` overrides it when set, a
     client's `show-mark` overrides the site when set for its own flow,
     unset inheriting at each tier, and `mark_enabled` carries the result
     so the page draws the wordmark, `logo.svg` beside `brand.logo_url`,
     there, because an issuer may want
     the mark on every site but one, and a site may want it on every flow
     but one;
     the hide is a page rule and not a refusal, the
     routes behind a hidden link stay open; because the flow that wants a
     plain page is one client's on a site whose other clients keep the
     full page, and a site key would strip every client at once.
