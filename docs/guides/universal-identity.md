...
| `events`                        | `{ "path": "/api/events", "topics": ["notifications", "session", "health", "admin"] }`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | the one stream of the events contract; `health` is the core topic every streaming UI backend sends for the footer's heart, and `admin` is the operator's topic (`restart-required`, `blocked-count`), answered `403` to anyone without `ROLE_ADMIN`; `/api/notifications/stream` and `notifications.js` retire with it, so a tab holds one connection and no page runs a timer |
