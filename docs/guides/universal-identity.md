...
| `mark_enabled`                                     | the parked client's `show-mark` when that key is set, true or false; else the site's own `sites.<id>.ui.show_mark` when set, true or false; else the issuer's `application.ui.show_mark`, false by default; each tier overrides the one above in either direction when set and inherits it when unset, so an issuer may want the mark on every site but one and a site on every flow but one; the page then draws the site's own mark, `brand.logo_url` of `/api/status`, above its sign-in heading and nothing else changes; sites for sure should have this, not just client apps                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `AuthMethodsApiController`, the parked client's `show-mark`, else the site's `ui.show_mark`, else `application.ui.show_mark` |
...
| `show-mark`                     | `mark_enabled`                                  | set true or false, draws or hides the site's mark above the sign-in heading for that client's flow, overriding the site's `ui.show_mark`, which when set overrides the issuer's `application.ui.show_mark` the same way; unset, the member inherits the site's answer, and the site's inherits the issuer's |
...
169. The sign-in page's hide switches are per client, never per site:
     ...
     so the page draws `brand.logo_url` there, because an issuer may want
