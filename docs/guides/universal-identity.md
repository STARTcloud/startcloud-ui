...
the identity feature imports no other feature, answering one
Configuration node at `/admin/config` whose children are one node per
...
does not, each a deep link to `/admin/config/<name>`, so the word
Configuration is drawn once in the column and a file is a place the
column names, never a strip the page draws (decision 122). The shared
...
122. The configuration editor's files are never tabs on the page: the
     page draws one file per route, `/admin/config/<name>`, and
     `/admin/config` the first name of `status.config`, that file's
     ...
     the row's place, one Configuration node at `/admin/config` whose
     ...
     Configuration, then the child's label; because a file is a place
     the column names and the column is the one navigation, and a tab
     strip beside it is a second one.
