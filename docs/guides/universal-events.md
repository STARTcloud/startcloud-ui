...
it proxies or holds the hub's unread count for the user; a UI backend that
answers `auth: []` has neither.
...
and `GET /api/health` once, and the stream carries every change after.
