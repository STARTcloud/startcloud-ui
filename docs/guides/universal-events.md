...
| `profile`       | `profile-updated`    | `{}`                                                             | none; the client reloads the session's profile, `GET /api/user`, on it  |
...
signed in; it reconnects when the adopted session changes, answers
...
connection, and answers `profile-updated` with the session's `reload()`
while a user is signed in, so the profile is re-read and
`useAccountPreferences` re-applies the look, theme, motion and language
it carries without a refresh. `events.path` is a same-origin path; a
...
| `useSessionKeepalive` answers `session-terminated` on the one stream with `events.endSession()` and `profile-updated` with the session's `reload()`      | n/a                         | ✓                                                                                          | ✓                                                                                             | to come                                                                                                                                               |
